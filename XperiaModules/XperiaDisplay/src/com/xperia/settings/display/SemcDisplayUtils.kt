...
class SemcDisplayUtils(private val context: Context) : IDisplayCallback.Stub() {
    ...
    companion object {
        ...
        const val VIDEO_ENHANCE_ENABLE = "video_image_enhance_enable"
    ...
    private val extLightService by lazy {
        ...
        ) ?: throw IllegalStateException("SEMC ExtLight AIDL not found")
    }
    ...
    fun isVideoImageEnhanceEnabled(): Boolean =
        Settings.Secure.getInt(context.contentResolver, VIDEO_ENHANCE_ENABLE, 0) != 0
    ...
    fun setVideoEnhanceEnabled(enabled: Boolean) {
        extLightService.setExtHdr(if (enabled) 1 else 0)
        Settings.Secure.putInt(context.contentResolver, VIDEO_ENHANCE_ENABLE, if (enabled) 1 else 0)
    }
